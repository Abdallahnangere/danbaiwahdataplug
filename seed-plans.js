...
async function main() {
  console.log("🌱 Seeding database with MTN data plans...");

  try {
    // First, delete existing data plans to avoid duplicates
    ...
    const mtnPlanCount = await prisma.plan.count({
      where: { network: "MTN" },
    });
    console.log(`📊 MTN plans: ${mtnPlanCount}`);
