...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log(`\n${'═'.repeat(80)}`);
    console.log(`📱 [DATA PURCHASE - API_B] New Request Started`);
    console.log(`📅 Timestamp: ${new Date().toISOString()}`);
    console.log(`🔗 Endpoint: /api/data/purchase-api-b`);
    ...
    console.log(`\n🌐 [SAIFUL API] Calling Saiful API_B...`);
    console.log(`📤 [SAIFUL REQUEST]:`);
    console.log(`   • plan: ${planDetails.externalPlanId}`);
    ...
    if (!result.success) {
      console.error(`❌ [SAIFUL] API call failed: ${result.message}`);
      return NextResponse.json(
